...
- 🧠 **团队经验共享** —— 某位成员的对话、结论与文件，会成为全团队可复用的知识。
- 👤 **按用户隔离** —— 每个用户都以专用系统账户 + 沙箱工作区运行独立 DSH，用户之间无法互访。
- 🖥️ **分屏工作台** —— 左侧 = 文件管理器 / 我的任务；右侧 = DSH 对话。可拖动分隔条，或点 ⤢ 在 30% ↔ 70% 间切换。
- 🔌 **单一入口、单一端口** —— 一个启动器（`:46001`）反向代理到每位成员的私有 DSH，对外只需暴露一个端口。
- ✅ **任务分配** —— 管理员按成员分配任务（富文本 + 图片 + 附件），统一存放在共享的 SQLite 中。
- 📁 **内置文件管理器** —— 上传、文件夹、树形浏览、压缩/解压、重命名、移动、删除、在线预览、下载。
- 🌐 **中英双语界面** —— 完整英文 / 中文（按钮、表格、状态、任务表单、登录页）。
- 🚀 **一键、离线安装** —— 复制文件夹即运行；无需编译、无需 .NET SDK、无需额外下载。
- 🧩 **升级不失效** —— 补丁层会在每次 DSH 升级后重新应用全部定制。
...
- 任务存放在启动器的共享 **SQLite** 库中（`<工作区>/database/taketopDSHTeam.db`；旧版按成员的 XML/DB 首次使用时自动导入）；附件存放在 `<工作区>/TaskData/Doc`。
