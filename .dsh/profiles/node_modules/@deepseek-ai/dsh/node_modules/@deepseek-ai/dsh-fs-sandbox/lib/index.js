...
var SandboxedFileSystem = class extends LocalFileSystem {
	...
	// The base sandbox only fences writes; reads pass through. These overrides
	// confine reads/stat too: any resolved target outside the session workspace
	// root is denied so a non-admin agent cannot browse the host filesystem
	// beyond its workspace.
	async stat(target, signal) {
		await this.enforceContained(target);
	...
	streamText(target, signal) {
		const sup = super.streamText(target, signal);
		return this.enforceContained(target).then(() => sup);
	}
	async enforceContained(target) {
		try {
			...
			if (root && !(await isPathUnder(target.targetKey, root))) {
				throw new FsError(`cannot access "${target.displayPath}": outside the sandboxed workspace`, "FS_SANDBOX_DENIED");
			}
		} catch (e) {
			if (e?.code === "FS_SANDBOX_DENIED") throw e;
		}
	}
	/**
