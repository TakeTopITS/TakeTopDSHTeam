...
function apply(ctx, config = {}) {
	...
	// PATCH: sandbox escalation is disabled entirely — a non-admin agent must not
	// be able to request a wider access mode (which would surface an approval
	// dialog and bypass workspace containment). Keep bash confined to the
	// workspace (defaultMode/sandboxPolicy stay effective) with no escalation.
	const escalationModes = []; // PATCH: no sandbox escalation
	...
	*/
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			// --- WORKSPACE CONTAINMENT PATCH (best-effort) ---
			// Guard so an agent cannot run commands outside its session workspace:
			// reject explicit workdir escapes and any drive-letter path in the
			// command that is not under the workspace root.
			const wsRoot = standingPolicy?.workspaceRoot;
			if (wsRoot && policy?.mode !== "danger-full-access") {
				const wanted = resolveWorkdir(args.workdir, exec, wsRoot);
				if (wanted !== void 0) {
					const nw = String(wanted).replace(/[\\/]+$/, "").toLowerCase();
					const nr = String(wsRoot).replace(/[\\/]+$/, "").toLowerCase();
					if (nw !== nr && !nw.startsWith(nr + "\\") && !nw.startsWith(nr + "/")) {
						throw new HarnessError(`cannot run in ${JSON.stringify(wanted)}: outside the sandboxed workspace`, "TOOL_DENIED");
					}
				}
				...
				if (/(?:^|[\\/])\.\.(?:[\\/]|$)/.test(cmd)) {
					throw new HarnessError("command escapes the sandboxed workspace (parent traversal)", "TOOL_DENIED");
				}
				// Reject any ABSOLUTE path reference not under the workspace root.
				// Handles Windows drive letters (C:\, D:\ …) and Unix rooted paths (/home, /tmp, …).
				const isOutside = (p) => {
					const np = String(p).replace(/[\\/]+$/, "").toLowerCase();
					return np !== nr && !np.startsWith(nr + "\\") && !np.startsWith(nr + "/");
				};
				const absPaths = cmd.match(/[A-Za-z]:[\\/][^'"\s]*|\/(?:[^'"\s])*/g) || [];
				for (const p of absPaths) {
					// Skip bare "/" and flags like "-/foo".
					if (p === "/" || p.startsWith("-") || p.startsWith("\"") || p.startsWith("'")) continue;
					if (isOutside(p)) throw new HarnessError(`command uses a path outside the sandboxed workspace: ${p}`, "TOOL_DENIED");
				}
				// Reject Windows UNC / rooted forms outside the workspace.
				if (/^[\\/]{2}[^\\/]+/.test(cmd.trim())) throw new HarnessError("command uses a path outside the sandboxed workspace (UNC/rooted path)", "TOOL_DENIED");
			}
