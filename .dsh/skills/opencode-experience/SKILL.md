...
metadata:
  docsRoot: "docs\\opencode-experience"
---

# OpenCode 工作经验数据（自动参考 + 数据归档）

本技能指向历史工作复盘目录，同时**经验数据目录也是以后工作数据的统一归档位置**。本 skill 与经验数据都位于 **TakeTopDshWeb 项目目录内**，整体拷贝 TakeTopDshWeb 即可在新电脑直接使用（无需用户级 .dsh）。处理任何与 TakeTop / TakeTopDshWeb / DshWeb 相关的任务前，**先读取经验数据**，可避免重复踩坑（SQL 迁移方言、hrow 表头、多语言 key、DSH 凭据格式等）。

## 0. 数据归档约定（用户 2026-09-03 规定，永久生效）

- **以后的工作数据一律存到 TakeTopDshWeb 项目内的经验目录 `docs\opencode-experience\`**（不含敏感凭据，如 API Key 一律不入档）。
...
以**当前 TakeTopDshWeb 项目根**为锚（不依赖盘符/驱动器，目录拷到任何位置都有效）：

1. 经验目录 = 项目根下 `docs\opencode-experience\`（即本 skill 文件上溯三级：`<项目根>\docs\opencode-experience`）。
2. 若项目根下不存在 `docs\opencode-experience`，提示用户该目录缺失，不要臆造内容。
