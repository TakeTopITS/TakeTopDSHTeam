# Licensing

TakeTopDSH Team is released under the **Business Source License 1.1 (BSL 1.1)**
(see [LICENSE](LICENSE) / [COPYING](COPYING)). BSL 1.1 is a **source-available**
license (it is not an OSI-approved open-source license) that automatically
converts to the **Apache License, Version 2.0** on the Change Date.

## Core terms

- **Source-available.** You may copy, modify, create derivative works,
  redistribute, and make non-production use of the software.
- **Additional Use Grant (free tier).** Production use is permitted **free of
  charge for an organization with up to 10 users**.
- **Above 10 users (tiered pricing).** A **commercial license** is required.
  Users are counted in **blocks of 10**: the first block (up to 10 users) is free,
  and **each additional block of 10 users costs USD 10 per year** — e.g. 11–20
  users = USD 10/year, 21–30 users = USD 20/year, and so on. If your use exceeds
  the grant you must purchase a commercial license, or refrain from using the
  software.
- **Automatic conversion.** On the Change Date (2030-09-11) — or the fourth
  anniversary of a given version's first public release, whichever is earlier —
  that version converts to **Apache-2.0** (fully open source). The license applies
  per version, so every new version has its own 4-year term.
- **Earlier MIT versions.** Releases previously published under the MIT License
  remain available under MIT permanently and are not affected by this change,
  which applies only to this and later versions.
- **Intellectual property** in this software is owned by
  泰顶拓鼎信息科技（上海）有限公司 (TakeTop Information Technology (Shanghai) Co., Ltd.)
  (Copyright (C) 2026-2036). All rights reserved.

## How "users" is counted

- A "user" is a named individual account authorized to use the software in
  production (the launcher's user accounts, including `admin`).
- Test / evaluation / non-production use does not count toward the limit.

## Commercial license

- For more than 10 users, or any use not covered by the Additional Use Grant,
  contact the company for a commercial license.
- **Pricing:** users are counted in **blocks of 10**. The first block (up to 10
  users) is free; **each additional block of 10 users is USD 10 per year**
  (e.g. 11–20 users = USD 10/year; 21–30 users = USD 20/year; and so on).
- Contact: **service@taketopits.com**
  (please state: intended use, deployment, and user count).

## Contributions

To keep the copyright complete (required to license and relicense the project),
external contributions must be made under a Contributor License Agreement (CLA)
assigning the relevant copyright to the company. Contributions without a CLA
cannot be merged. See `CONTRIBUTING.md`.

## Third-party components

This product bundles third-party components under their own licenses, for
example: `docx-preview` (Apache-2.0), `ExcelJS` (MIT), `JSZip` (MIT),
`@aiden0z/pptx-renderer` (Apache-2.0), and the DeepSeek Harness runtime
`@deepseek-ai/*` (MIT). They remain under their own licenses; their notices are
kept in `gui-cs/src/wwwroot/vendor/licenses/` and `NOTICE`, and must be retained
when redistributing.

> Disclaimer: this document is a licensing notice only and is not legal advice;
> the binding terms are those of the LICENSE file and any signed agreement.
