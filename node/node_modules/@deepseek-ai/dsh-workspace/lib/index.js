...
var WorkspaceEntity = class {
	...
	*/
	...
	get sessionIds() {
		return this.record.sessionIds.filter((id) => sameWorkspacePath(this.host.sessionPath(id), this.record.path));
	}
	...
	async attachSession(sessionId) {
		if (!this.record.sessionIds.includes(sessionId)) {
			...
			if (!sameWorkspacePath(cwd, this.record.path)) throw new Error(`cannot attach session '${sessionId}' to workspace '${this.record.path}': its cwd resolves to '${cwd}'`);
	...
	async mutate(fn) {
		...
		try {
			next = await this.host.table().update(this.id, (current) => {
				...
				const sessionIds = changed.sessionIds.filter((id) => sameWorkspacePath(this.host.sessionPath(id), changed.path));
