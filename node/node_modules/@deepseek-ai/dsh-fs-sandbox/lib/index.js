...
* text-storage mechanics â€?resolve, stat, read/stream, list, the atomic
* write and the read-match-write edit critical section â€?are the local
...
* NOT a kernel boundary â€?the operations are the seam's own (open, rename),
...
* swap â€?the model-facing tools are untouched). Its configured default mode is
...
var SandboxedFileSystem = class extends LocalFileSystem {
	...
	/** The deployment default mode â€?the capability fact the tool layer reads to advertise escalation. */
	get sandboxMode() {
...
async enforceContained(target) {
	...
	* refusal â€?the tool layer maps it to the model-facing `[sandbox: â€¦]` marker
