...
async function runRipgrep(ctx, exec, toolName, argv, rawOutputMaxBytes, graceMs, stderrMaxBytes) {
	...
	// Force the search root to the session workspace: any argv element that is an
	// absolute path escaping the workspace is clamped to the workspace root, so a
	// non-admin agent cannot search the host filesystem beyond its workspace.
	...
	if (workdir && exec.agent?.session.header.cwd) {
		...
		containedArgv = argv.map((a) => {
			...
			// Only consider path-like absolute args (drive/UNC or leading slash).
			const isAbs = /^[A-Za-z]:[\\/]/.test(trimmed) || /^[\\/]{2}[^\\/]+/.test(trimmed) || /^[\\/]/.test(trimmed);
			if (!isAbs) return a;
			const c = trimmed.replace(/[\\/]+/g, "\\").replace(/[\\/]$/, "").toLowerCase();
			if (c === wc || c.startsWith(wc + "\\")) return a; // already inside
			// Outside the workspace: clamp to the workspace root.
			return ".";
		});
	}
	let handle;
