...
> 👥 **适合谁用：** 已经在用（或想用） AI agent，但需要**多用户能协同工作、又能按用户隔离、数据完全自控**的团队 —— 既不想给每个人各装一个  AI agent，也不想把数据交给云服务。
...
![启动器控制台 —— 启动/停止默认 DSH、设置工作区](assets/launcher.png)

- **新建实例** —— 自动分配工作区（`<全局工作区>/<用户名>`）、专用系统账户 + 沙箱。
- 对每个成员可 **启动 / 停止 / 打开 / 删除 / 重置密码**。
- ![成员视图 —— 创建并管理成员](assets/AddMember.png)
