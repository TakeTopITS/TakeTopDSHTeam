...
TakeTopDSHTeam 把 DeepSeek Harness（DSH）变成面向团队的多用户 AI 协同工作平台 —— 一个自托管的网页工作区，适用行业不限（软件开发，工程设计，文档编辑，媒体处理等等）。每位成员拥有独立的分屏工作台，内置隔离的 DSH，文件管理、任务管理功能；对话、文件与任务在团队内传递，知识持续沉淀、效率不断提升。支持多种大语言模型（DeepSeek、OpenAI、Claude、Kimi、Local Model等），所有成员能共用一个 API LLM Key —— 好管理、更省钱！
...
> 🔒 **自托管设计——不是云服务。** 我们**不**提供任何云端托管或租赁服务。你下载安装包并在自己的电脑或服务器上本地安装；账号、会话、文件与 API LLM Key 都保存在你自己的磁盘上，我们从不接收或保存你的数据——数据完全由你掌控。（唯一的对外连接是你自己配置的大模型服务商。）

> **MIT 许可（开源）。** 本项目以 **MIT License** 授权（见 [LICENSE](LICENSE) / [COPYING](COPYING)）——你可以自由使用、修改与分发（包括商业用途），只需保留版权声明。若团队规模更大、需要**按成员控制 Token 用量**与**金字塔式成员管理**，我们还提供**收费的高级版**（见下文）。
...
> 📦 **安装包为什么有 500 多 MB？** 这是为了方便用户一键安装：发行包**打包了在三大操作系统（Windows、macOS、Linux）上安装所需的全部文件**——各平台自包含的启动器、Node.js 运行时，以及离线的 `@deepseek-ai/dsh` 依赖包。下载一次，即可在任一系统上**离线一键安装**，无需再下载、无需在目标机器上编译。
...
每位成员还有一个简洁的**管理页**页面 —— 启动/停止自己的 DSH、打开它、查看该成员的日志：

![成员视图 —— 我的工作日志](assets/MemberLauncher.png)

- **新建用户** —— 自动分配工作区（`<全局工作区>/<用户名>`）、专用系统账户 + 沙箱。
...
- **继续分派** —— 从当前任务创建后续（子）任务并选择**受理人**；子任务归入该成员的"我的任务"，保存后父任务自动展开显示它。
- **编辑 / 删除** —— 自己创建的任务行会显示"编辑/删除"按钮（仅限自己的任务）。
- **子任务树** —— 子任务列的 `+` 可内联展开该任务的子任务（缩进、支持多层）；没有子任务的任务不显示 `+`。
...
- **添加 / 编辑** 任务：**受理人**、**任务类型**（类型下拉框 + `+` 可新增类型）、**任务名称**、**内容**（富文本——加粗/列表/引用，可粘贴或插入图片）、**分派时间**、**状态**（处理中 / 完成 / 取消）、**相关文件**（可多选上传）。
- **子任务层级** —— 每行任务都有一个 `+` 列；点击可内联展开该任务的子任务（缩进、支持多层）。子任务把父任务的**序号**存为"上级任务"（缺省 `0` 表示顶层）；没有子任务的任务不显示 `+`。
- 任务存放在启动器的共享**数据库**中（`<工作区>/database/taketopDSHTeam.db`）；附件存放在 `<工作区>/TaskData/Doc`。

![任务分配 —— 选择成员后添加/编辑其任务](assets/TaskAssignment.png)

![添加任务 —— 填写受理人 / 类型 / 名称 / 内容 / 时间 / 状态 / 相关文件](assets/AddTask.png)

![任务分配 —— 用"+"列内联展开子任务](assets/TaskSubtask.png)
...
本仓库是**标准版**，采用 **MIT 许可、免费**。若团队需要更强的管控能力，我们提供**收费的高级版**，额外增加：

- **🧮 按成员控制 Token 用量** —— 可查看并控制每个成员的 Token 用量。
- **🏛️ 金字塔式成员管理** —— 普通成员可以创建并管理子成员的工作，子成员还可以继续创建子成员并管理他们的工作，形成金字塔式的管理架构。
...
- 我们也提供**收费的高级版**（见上文[高级版](#高级版收费)）；其商业条款见 [LICENSE-COMMERCIAL.md](LICENSE-COMMERCIAL.md)。
...
版权所有 (c) 2026-2036 泰顶拓鼎信息科技（上海）有限公司 (TaiDingTuoDing Information Technology (Shanghai) Co., Ltd.)。保留所有权利。
